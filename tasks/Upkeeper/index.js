...
exports.handler = async function (credentials, context) {
    ...
    const contract = new ethers.Contract(LZ_RATE_PROVIDER_POKER_ADDRESS, LZ_RATE_PROVIDER_POKER_ABI, signer);
    ...
    // performUpkeep
    if (upkeepNeeded) {
        // fetch the last time the upkeep was performed
        const lastRun = await contract.connect(signer).LastRun();
        // calculate how long it has been since the last upkeep
